...
class AngularEntropy:
    ...
    def __call__(self, logits: torch.Tensor, return_diag: bool = False):
...
def compute_gradient_stats(params: List[torch.nn.Parameter]) -> Tuple[float, bool, bool]:
    ...
    Concatenating all gradients can OOM for large models (e.g., 8B) even when the
    optimization itself fits. We instead reduce per-parameter.
    """
    ...
    for p in params:
        ...
        # Scalar norm; internal reduction avoids materializing a flat buffer.
        gn = torch.norm(g)
