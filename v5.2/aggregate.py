"""Aggregate v5.2 results and generate comparison charts/tables."""
...
def plot_bars(rows: List[Dict], out_path: str) -> None:
    ...
    # keep stable order
    rows = sorted(rows, key=lambda r: r["token_acc_after"], reverse=True)
...
def plot_scatter(rows: List[Dict], out_path: str) -> None:
    ...
    y = [r.get("token_acc_after", 0.0) * 100 for r in rows]
    ...
    for r in rows:
        ax.annotate(r.get("method", ""), (r.get("trainable_pct", 0.0), r.get("token_acc_after", 0.0) * 100), fontsize=8, alpha=0.85)

    ax.set_xlabel("Trainable Params (%)")
    ax.set_ylabel("Token Accuracy After (%)")
    ax.set_title("Accuracy vs. Trainable Subset (color=runtime minutes)")
...
def parse_args():
    p = argparse.ArgumentParser(description="Aggregate v5.2 results")
