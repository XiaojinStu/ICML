"""v8.3 main experiment runner (Greedy vs Greedy+ANE).

Supported datasets (for now):
- addition_50 (local JSON)
- gsm8k (local JSON)
- bigbench_crt (BIG-bench chinese_remainder_theorem)
    ...
    *,
    ...
    if save_mode == "compact":
        ...
        def _trim_metrics(m: Dict, keep_full: bool) -> Dict:
            ...
            keep_keys = {"ane", "target_prob", "target_rank", "step_status"}
            out = {k: m[k] for k in keep_keys if k in m}
            if keep_full:
                for k in ["tracked_topk", "num_topk_snapshots", "num_topk_probs"]:
                    if k in m:
                        out[k] = m[k]
            return out
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="v8.3 main runner (Greedy vs Greedy+ANE)")

    p.add_argument("--dataset", default="bigbench_crt", help="addition_50 | gsm8k | bigbench_crt (others reserved)")
...
def main() -> None:
    ...
    s = output["summary"]
    print(
        f"[v8.3] dataset={s['dataset']} mode={s['eval_mode']} model={s['model']} "
