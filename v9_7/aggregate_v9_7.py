"""Aggregate v9.7 main experiment outputs into a paper-ready table + curves.

Reads v9 JSON outputs under a root folder (typically `runs/v9.7_main_final/results`).
Writes:
- `table_main_v9_7.csv`
...
def plot_curves(rows: List[Dict[str, Any]], out_dir: Path, checkpoints: List[int]) -> None:
    ...
    def _plot_group(group_models: List[str], out_name: str) -> None:
        if not group_models:
            return
        fig, axes = plt.subplots(len(datasets), 1, figsize=(9.6, 3.6 * len(datasets)), squeeze=False)
        for i, ds in enumerate(datasets):
            ...
            for model in group_models:
                ...
                if not cand:
                    continue
                cand.sort(key=lambda r: float(r.get("token_acc_after", 0.0)), reverse=True)
                r = cand[0]
                ys = np.array([float(r.get(f"token_acc@{s}", 0.0)) for s in checkpoints], dtype=float)
                label = f"{model} ({r.get('phase')}, lr={r.get('lr')})"
        ...
        fig.tight_layout()
        fig.savefig(out_dir / out_name, facecolor="white")
        plt.close(fig)
...
def main() -> None:
    ...
    write_csv(rows, out_dir / "table_main_v9_7.csv", fields)
    plot_curves(rows, out_dir / "plots", checkpoints)
    print(f"[v9.7] Wrote: {out_dir/'table_main_v9_7.csv'}")
    print(f"[v9.7] Plots: {out_dir/'plots'}")
