...
def make_optimizer(params: List[torch.nn.Parameter], lr: float, optimizer: str = "sgd") -> torch.optim.Optimizer:
    if optimizer == "adamw":
...
class TTAEngine:
    ...
    def run_token(self, input_ids: torch.Tensor, target_idx: int) -> Tuple[int, int, Dict]:
        ...
        optimizer = make_optimizer(self.params, self.lr, self.optimizer)
