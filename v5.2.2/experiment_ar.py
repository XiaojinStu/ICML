...
def evaluate_one_config(
    *,
    ...
    summary = {
        ...
        "token_topk_acc_before": {str(k): topk_correct_before[k] / token_total if token_total > 0 else 0 for k in topk_list},
        # "token_topk_acc_after": {str(k): topk_correct_after[k] / token_total for k in topk_list},
        "token_topk_acc_after": {str(k): topk_correct_after[k] / token_total if token_total > 0 else 0 for k in topk_list},
        "seq_total": seq_total,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # p.add_argument("--dataset", default="SVAMP")
    p.add_argument("--data_path", default="..//data/SVAMP")
    p.add_argument("--data_split", choices=["train", "test"], default="test", help="Which split to load when using gsm8k folder")
...
def main():
    ...
    # Load dataset: support JSON file (old behavior) or local gsm8k parquet folder
    if os.path.isdir(args.data_path):
        # expect structure like <data_path>/data/train-00000-of-00001.parquet
        split = args.data_split
        parquet_path = os.path.join(args.data_path, "data", f"{split}-00000-of-00001.parquet")
        data = []
        if os.path.exists(parquet_path):
            try:
                ...
                d = table.to_pydict()
                qs = d.get("question", [])
                ans_field = d.get("answer", [])
                for q, a in zip(qs, ans_field):
                    s = str(a)
                    m = re.search(r"####\s*([+-]?\\d+)", s)
                    if m:
                        ans = int(m.group(1))
                        ...
                        ans = int(float(nums[-1])) if nums else None
                    data.append({"question": q, "answer": ans})
            except Exception:
                # fallback to modelscope loader (lazy import to avoid module import errors)
                try:
                    from modelscope.msdatasets import MsDataset

                    ds = MsDataset.load("modelscope/gsm8k", subset_name="main", split=split)
                except Exception:
                    ds = []
                for item in ds:
                    q = item.get("question")
                    s = str(item.get("answer", ""))
                    m = re.search(r"####\s*([+-]?\\d+)", s)
                    if m:
                        ans = int(m.group(1))
                    else:
            ...
            # try to load using modelscope api (will use cache or remote)
            try:
                from modelscope.msdatasets import MsDataset
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, dtype=dtype, device_map=args.device_map) #`torch_dtype` is deprecated! Use `dtype` instead!
