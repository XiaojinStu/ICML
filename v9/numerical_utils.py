"""Numerical token utilities for v9."""
...
def _is_digit_token(token: str, allow_prefix_space: bool) -> bool:
    ...
    if allow_prefix_space:
        token = token.strip()
    # NOTE: `str.isdigit()` also matches many unicode digits (e.g., Arabic-Indic),
    # which can cause confusing token sets and plotting font issues.
...
def get_numerical_tokens(
    ...
    """Return (token_ids, token_strings, int_values) for pure-digit tokens."""
...
def numerical_shannon_entropy(logits: torch.Tensor, num_idx_tensor: torch.Tensor, eps: float = 1e-12) -> float:
    """Shannon entropy over numerical subset (for analysis / future baselines)."""
    probs = numerical_softmax(logits, num_idx_tensor)
