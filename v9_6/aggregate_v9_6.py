    ...
    def _plot_group(group_models: List[str], out_name: str) -> None:
        ...
        for i, ds in enumerate(datasets):
            ...
            for model in group_models:
                ...
                if not cand:
                    continue
                # If multiple phases exist for same ds/model, pick the one with max token_acc_after
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Aggregate v9.6 runs")
...
def main() -> None:
    ...
    for path in find_json(root):
        ...
        # phase inferred from folder structure: root/<phase>/...
        try:
    ...
    write_csv(rows, out_dir / "table_main_v9_6.csv", fields)
    plot_curves(rows, out_dir / "plots", checkpoints)
    print(f"[v9.6] Wrote: {out_dir/'table_main_v9_6.csv'}")
    print(f"[v9.6] Plots: {out_dir/'plots'}")
